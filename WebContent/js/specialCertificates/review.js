...
layui.use([ 'laypage', 'layer', 'table', 'element', 'slider', 'commons', 'combo', 'form', 'upload', 'laydate' ], function() {
	...
	form.on('submit(submitEdit)', function(data) {
		var url = "update.do";
		var date = {"status": data.field.status,"id":data.field.id};
		...
		switch (obj.event) {
			...
			case 'edit':
				...
				data.files.forEach(function (value) {
					var htm = '<img style="width:150px;height: 100px;" src="downloadimg.do?path='+ value.url+'" onclick="previewImg(this)" />';
					if (value.fileType === 0){
						$('#idCardImgEdit').append(htm);
					}else if (value.fileType === 1){
						$('#educationCertificateImgEdit').append(htm);
					}else if (value.fileType === 2){
						$('#otherImgEdit').append(htm);
					}
				});
				...
				idx = layer.open(
					{
						...
						cancel : function() {
							$('#frm1Edit').hide();
							$('#idCardImgEdit').empty();
							$('#educationCertificateImgEdit').empty();
							$('#otherImgEdit').empty();
						}
					});
				break;
