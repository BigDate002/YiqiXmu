...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function(e) {
	...
	F.prototype.config = {
		...
		if(i.startTime = (new Date).getTime(), a.url) {
			...
			if(i.config.beforeLoad){
				i.config.beforeLoad(d);
			}
		...
		a.elem.on("click", 'input[name="layTableCheckbox"]+', function() {
			var e = t(this).prev(),
				...
				if(n&&l!==undefined){
					selections.indexOf(l)<0&&selections.push(l);
				}else{
					selections.remove(l);
				}
		...
		return layui.each(l, function(e, l) {
			return l.constructor === Array ? void i++ : void(l[d.config.checkName] && (t++, a.push(l)))
		}), {
			data: a,
